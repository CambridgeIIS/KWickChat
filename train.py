...
def get_data_loaders(args, tokenizer):
        ...
        for dialog in dataset:
            ...
            for _ in range(args.personality_permutations):
                for utterance in dialog["utterances"]:
                    ...
                    keyphrase = utterance["key"]
                    for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                        lm_labels = bool(j == num_candidates-1)
                        # keyphrase = keyphrase_extraction(history)
                        instance = build_input_from_segments(persona, history, candidate, tokenizer, keyphrase, lm_labels)
    ...
    for dataset_name, dataset in datasets.items():
        ...
        for input_name in MODEL_INPUTS:
            ...
            # print(input_name)
            # tensor = torch.tensor([int(s) for s in dataset[input_name]])
